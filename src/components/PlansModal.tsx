...
import { X, Check, Zap, Users, Loader2, Star } from "lucide-react";
      ...
      {open && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-5xl bg-white dark:bg-zinc-900 rounded-3xl border border-gray-200 dark:border-zinc-800 shadow-2xl max-h-[92dvh] flex flex-col overflow-hidden"
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-zinc-800 shrink-0">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 dark:text-zinc-500">Cada plano tem uma moldura exclusiva no seu avatar</p>
                </div>
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto scrollbar-thin p-5 sm:p-6">
                  ...
                  return (
                    <div
                      ...
                      className={`relative rounded-2xl border p-5 flex flex-col ${
                        plan.highlight
                          ? "border-amber-400/50 shadow-lg shadow-amber-500/10 bg-amber-500/[0.03]"
                          : "border-gray-200 dark:border-zinc-800"
                      }`}
                    >
                      {plan.highlight && (
                        <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[10px] font-bold uppercase tracking-wide shadow-sm flex items-center gap-1">
                          <Star className="w-3 h-3" /> Melhor oferta
                        </span>
                      )}
                      {plan.group && (
                        <span className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-full bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wide shadow-sm flex items-center gap-1">
                      ...
                      <div className="flex justify-center mb-3">
                        <PlanAvatar photoUrl={currentUser?.photoUrl} plan={id} size={52} showGem={plan.paid} />
                      </div>

                      <h3 className="text-lg font-bold text-gray-900 dark:text-zinc-50 text-center">{plan.name}</h3>
                      <p className="text-[11px] text-gray-400 dark:text-zinc-500 mb-3 min-h-[2.5rem] text-center">{plan.tagline}</p>
                      ...
                      <button
                        ...
                        title={plan.paid ? "Pagamento via Stripe (ver BILLING.md)" : undefined}
                        className={`w-full px-4 py-2.5 rounded-xl text-sm font-semibold transition-all cursor-pointer disabled:cursor-default disabled:opacity-60 flex items-center justify-center gap-2 ${
                          isCurrent
                            ...
                            : plan.highlight
                              ? "bg-gradient-to-r from-amber-400 to-orange-500 text-white hover:opacity-95 shadow-sm"
                      ...
                      >
                        ...
                        {isCurrent ? "Plano atual" : id === "free" ? "Começar grátis" : `Assinar ${plan.name}`}
                      </button>
                    </div>
                  );
                })}
              ...
              <p className="text-center text-[11px] text-gray-400 dark:text-zinc-600 mt-5">
                Preços de lançamento. Cancele quando quiser. Cobrança via Stripe (ver BILLING.md).
              </p>
            </div>
          </motion.div>
        </motion.div>
